...
///    its items.
///  * [MySliverReorderableList], a sliver list that allows the user to reorder
///    its items.
...
/// [ReorderableList] and [MySliverReorderableList].
///
...
class ReorderableListState extends State<ReorderableList> {
  final GlobalKey<MySliverReorderableListState> _sliverReorderableListKey = GlobalKey();
    ...
    return CustomScrollView(
      ...
      slivers: <Widget>[
        SliverPadding(
          ...
          sliver: MySliverReorderableList(
...
/// [MySliverReorderableListState.startItemDragReorder]. This is most easily
...
/// item drag method.
///
/// This widget's [MySliverReorderableListState] can be used to manually start an item
/// reorder, or cancel a current drag that's already underway. To refer to the
/// [MySliverReorderableListState] either provide a [GlobalKey] or use the static
/// [MySliverReorderableList.of] method from an item's build method.
///
/// See also:
///
...
class MySliverReorderableList extends StatefulWidget {
  ...
  @override
  MySliverReorderableListState createState() => MySliverReorderableListState();

  /// The state from the closest instance of this class that encloses the given
  /// context.
  ///
  /// This method is typically used by [MySliverReorderableList] item widgets to
  /// start or cancel an item drag operation.
  ///
  /// If no [MySliverReorderableList] surrounds the context given, this function
  /// will assert in debug mode and throw an exception in release mode.
  ///
  /// This method can be expensive (it walks the element tree).
  ///
  ...
  ///
  ///  * [maybeOf], a similar function that will return null if no
  ///    [MySliverReorderableList] ancestor is found.
  static MySliverReorderableListState of(BuildContext context) {
    final MySliverReorderableListState? result = context.findAncestorStateOfType<MySliverReorderableListState>();
    assert(() {
      if (result == null) {
        throw FlutterError.fromParts(<DiagnosticsNode>[
          ErrorSummary(
  ...
  /// context.
  ///
  /// This method is typically used by [MySliverReorderableList] item widgets that
  /// insert or remove items in response to user input.
  ///
  /// If no [MySliverReorderableList] surrounds the context given, this function
  /// will return null.
  ...
  /// See also:
  ///
  ///  * [of], a similar function that will throw if no [MySliverReorderableList]
  ///    ancestor is found.
  static MySliverReorderableListState? maybeOf(BuildContext context) {
    return context.findAncestorStateOfType<MySliverReorderableListState>();
  }
}
...
///
/// An app that needs to start a new item drag or cancel an existing one
/// can refer to the [MySliverReorderableList]'s state with a global key:
///
/// ```dart
...
/// refer to their [MySliverReorderableList] with the static
/// [MySliverReorderableList.of] method.
DragInfo? _dragInfo;
Offset _startingPostion = Offset.zero;

class MySliverReorderableListState extends State<MySliverReorderableList> with TickerProviderStateMixin {
  ...
  @override
  void didUpdateWidget(covariant MySliverReorderableList oldWidget) {
  ...
  void _dragUpdate(DragInfo item, Offset position, Offset delta) {
       setState(() {
  ...
  void _dragCancel(DragInfo item) {
    setState(() {
      _dragReset();
    });
  }

  void _dragEnd(DragInfo item) {
    setState(() {
      if( _extentOffset(item.itemExtent, _scrollDirection) >= Offset(item.itemExtent, 50)){
    ...
    _startingPostion = _finalDropPosition!;
  ...
  void _dragUpdateItems() {
    ...
    // Check if the dragged item is outside the specified bounds
    if (_startingPostion.dy.toInt() + 60 < _dragInfo!.dragPosition.dy.toInt() ||
        _dragInfo!.dragPosition.dy.toInt() < _startingPostion.dy.toInt() - 60) {
      // Reset the gap for all items since the dragged item is out of bounds
      //for (final _ReorderableItemState item in _items.values) {
      //  item.resetGap();
      //}
      _resetItemGap();
      // Exit early as we don't need to update the insert index
      return;
    }
    ...
    for (final _ReorderableItemState item in _items.values) {
      ...
      if (_reverse) {
        ...
        if((_startingPostion.dy.toInt()+60 < _dragInfo!.dragPosition.dy.toInt() || _dragInfo!.dragPosition.dy.toInt() < _startingPostion.dy.toInt()-60 )){
          // The start of the proxy is out of the dock box , so
          // we should not swap the item with the gap and we are done looking for
          // the same index.
...
DragInfo? currentDragInfo=null;

class _ReorderableItemState extends State<_ReorderableItem> {
  late MySliverReorderableListState _listState;
  ...
  @override
  void initState() {
    _listState = MySliverReorderableList.of(context);
  ...
  Widget build(BuildContext context) {
    if (_dragging) {
      final Size size = _extentSize(_dragInfo!.itemExtent, _listState._scrollDirection);
      return (_startingPostion.dy.toInt()+60 < _dragInfo!.dragPosition.dy.toInt() || _dragInfo!.dragPosition.dy.toInt() < _startingPostion.dy.toInt()-60 ) ?SizedBox(width: 1,height: size.height,) :SizedBox.fromSize(size: size);
    }
    print("insert index ${_listState._insertIndex}, widget index = ${_listState._dragIndex}");
      _listState._registerItem(this);
...
class ReorderableDragStartListener extends StatelessWidget {
  ...
  void _startDragging(BuildContext context, PointerDownEvent event) {
    ...
    final MySliverReorderableListState? list = MySliverReorderableList.maybeOf(context);
...
class DragInfo extends Drag {
  ...
  late MySliverReorderableListState listState;
  late int index;
  late Widget child;
...
class _DragItemProxy extends StatelessWidget {
  ...
  final MySliverReorderableListState listState;
  final int index;
  final Widget child;
...
class _ReorderableItemGlobalKey extends GlobalObjectKey {
  ...
  final Key subKey;
  final int index;
  final MySliverReorderableListState state;
